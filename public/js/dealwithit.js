...
if (getQueryVariable('link')) {
    ...
    $('#text').html('');
    scalar = 1;
    $('form').replaceWith('<form action="http://stevetest12.azurewebsites.net/"><input type="submit" id="redirect" value="Wanna make another?"> <input type="button" id="twitter" value="Share on twitter">');
...
$('body').on('click', "#twitter", function () {
    window.open('https://twitter.com/intent/tweet?text=' + encodeURIComponent('Deal with it... ' + encodeURI('http://stevetest12.azurewebsites.net/index.html?link=') + encodeURI(origimgurl)) + '&hashtags=stevetest12');
